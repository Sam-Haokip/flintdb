...
- `PRIMARY KEY` is single-column only. Exactly zero or one `PRIMARY KEY` per table. Every table's rows — with or without a `PRIMARY KEY` — live in an unordered heap file (Phase 1 baseline); a `PRIMARY KEY` additionally gets an automatically-created, uniqueness-enforced B+-tree index on that column, giving fast point/equality lookups by primary key but **not** physical row ordering — `SELECT *` returns rows in heap order regardless of whether the table has a primary key. (An earlier draft of this bullet described the primary key as the table's true clustering key, with rows physically stored in primary-key order in the B+-tree itself; that was revised once implementation reached the point of actually building it — see `docs/DECISIONS.md` D-040 for why a unique index over the existing heap file was chosen instead of generalizing the B+-tree's fixed-size leaf format to variable-length row payloads.)
- `CREATE INDEX` builds one secondary B+-tree on exactly one column. Secondary indexes are non-unique (duplicate keys allowed) and store the row's heap RID as the indexed value, not the full row, and not the primary key either. (An earlier draft of this bullet said a secondary index stores "the primary key (or heap row-id, for PK-less tables)" — a two-hop design, secondary index → PK value → PK index → RID, that made sense only under the original clustered-PK design this document itself has since abandoned: see `docs/DECISIONS.md` D-040, which made a `PRIMARY KEY` an ordinary unique index over the heap file rather than the table's physical storage order. Once the PK index is itself just `BPlusTree<pk_value, RID>`, every index — PK or secondary — storing `RID` directly is both simpler and exactly what's already implemented (`index/btree.h`'s `BPlusTree::Insert(int64_t key, RID rid)` has never taken anything else); see D-048 for where this staleness was caught and corrected.)
...
**Known limitation, found and measured in Phase 7 (`docs/DECISIONS.md` D-055, `docs/BENCHMARK_PHASE7.md` §4), not a correctness gap but a severe concurrency *performance* one:** `BPlusTree::Insert`/`InsertRecursive` exclusively lock every node from an index's actual root down to the target leaf, through the same page-level `LockManager` as everything else, held until commit like every other lock under Strict 2PL. This is correct — it produces genuinely serializable results — but it means every insert into a given index contends on the same root-node lock regardless of tree depth or how far apart two inserts' target keys are, which measured as a two-to-three-order-of-magnitude throughput collapse at 8 concurrent inserting threads. The real fix is lock coupling / latch crabbing (separating the tree's physical structural protection from its logical transactional protection so a node's latch can be released the moment a traversal proves it's safe from splitting), deliberately not attempted in Phase 7 as too large and correctness-sensitive a redesign to rush — see D-055 for the full reasoning and the measurements.
...
3. Whether no-steal survives contact with a real benchmark workload, or needs to become steal+undo — Phase 3/7. **Checked in Phase 7, still open:** the benchmark workload (`docs/DECISIONS.md` D-055) never dirtied more pages per transaction than comfortably fit in memory, so it never actually stressed the dimension this question is about. `BufferPool` has no eviction at all (a Phase-1 simplification, `src/storage/buffer_pool.h`), so there is no configured capacity to overflow today — a transaction that dirtied enough pages to matter would show up as unbounded memory growth, not a clean rejection, and nothing in this project has built or run a workload designed to actually find that limit. Carried forward to Phase 8 as a known, unresolved limitation rather than treated as settled.
